...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        fig = px.pie(filtered_df, values='class', names='Launch Site', title='Success Count for All Sites')
    else:
        filtered_df = filtered_df[filtered_df['Launch Site'] == entered_site]
        fig = px.pie(filtered_df, values='class', names='Launch Site', title=f'Success Count for {entered_site}')
    return fig
...
def update_scatter_plot(entered_site, payload_range):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.scatter(
            filtered_df, x='Payload Mass (kg)', y='class', 
            color='Booster Version Category',
            title=f'Success by Payload and Booster Version for {entered_site}'
        )
    return fig
